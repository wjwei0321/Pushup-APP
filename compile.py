...
# 1. Update API URL and remove from settings
js = js.replace(
...
s_saveSettings = """function saveSettings() {
...
r_saveSettings = """function saveSettings() {
    const emailVal = document.getElementById('userEmail').value.trim();
    if (emailVal) {
        userEmail = emailVal;
        localStorage.setItem('pushup_userEmail', userEmail);
...
js = js.replace(s_saveSettings, r_saveSettings)

# 2. Update fetchData (normalization and mapping)
s_fetchData = """            trainingData = json.data.filter(row => row[0]).map((row, index) => {
...
r_fetchData = """            trainingData = json.data.map(row => {
                let dStr = row[0];
...
js = js.replace(s_fetchData, r_fetchData)
...
# 4. updateModalStats
s_modalStats = """function updateModalStats() {
...
r_modalStats = """function updateModalStats() {
    if (!selectedExerciseForLog) return;
    const dateStr = `${selectedDate.getFullYear()}/${String(selectedDate.getMonth()+1).padStart(2, '0')}/${String(selectedDate.getDate()).padStart(2, '0')}`;
...
js = js.replace(s_modalStats, r_modalStats)

# 5. submitWorkout
s_submit = """async function submitWorkout() {
...
r_submit = """async function submitWorkout() {
    if (!selectedExerciseForLog) return alert('Please select an exercise.');
    const reps = document.getElementById('workoutRepsDisplay').textContent;
    if (!reps || isNaN(reps) || parseInt(reps) <= 0) return alert('Please enter valid reps.');
...
r_inline = """function saveInlineEdit(input) {
    input.setAttribute('readonly', 'true');
    input.style.borderBottom = 'none';
...
js = js.replace(s_inline, r_inline)

s_updateSet = """function updateSetOnBackend(entry) {
    showSplashScreen();
...
r_updateSet = """function updateSetOnBackend(entry) {
    showSplashScreen();
    const payload = {
        action: 'edit',
        rowIndex: entry.rowIndex,
...
r_delete = """async function confirmDeleteSet(rowIndex) {
    const entry = trainingData.find(d => d.rowIndex === rowIndex);
    if (!entry) return;
...
js = js.replace(s_delete, r_delete)

# 7. renderDailyLog
s_dailyLog = """    dayData.forEach(entry => {
...
js = js.replace(s_dailyLog, r_dailyLog)

# 8. renderStats
s_stats = """    filteredData.forEach(d => {
...
r_stats = """    filteredData.forEach(d => {
        if (!dailyTotals[d.dateStr]) dailyTotals[d.dateStr] = { sets: [] };
