...
render_pattern = re.compile(r'    dayData\.forEach\(entry => \{\s*entry\.sets\.forEach\(\(repCount, setIndex\) => \{.*?\s*\}\);\s*\}\);', re.DOTALL)
...
js = render_pattern.sub(new_render, js)

# 2. Update saveInlineEdit
save_pattern = re.compile(r'function saveInlineEdit\(input\) \{.*?\n\}\n', re.DOTALL)
new_save = """function saveInlineEdit(input) {
...
"""
js = save_pattern.sub(new_save, js)

# 3. Update updateSetOnBackend
update_pattern = re.compile(r'function updateSetOnBackend\(entry\) \{.*?\n\}\n', re.DOTALL)
new_update = """function updateSetOnBackend(entry) {
    showSplashScreen();
    const payload = {
        action: 'edit',
...
"""
js = update_pattern.sub(new_update, js)

# 4. deleteSet and signatures
...
delete_pattern = re.compile(r'async function deleteSet\(\) \{.*?\n\}\n', re.DOTALL)
new_delete = """async function deleteSet() {
...
"""
js = delete_pattern.sub(new_delete, js)
